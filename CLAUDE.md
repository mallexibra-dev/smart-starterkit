...
Smart Starterkit is a modern monorepo template built with Bun, featuring a React frontend with TanStack Router and a Hono backend with MySQL database. The project uses TypeScript throughout and follows a modular architecture with shared types and utilities.
...
```
server/src/
├── route/          # API route definitions with Swagger docs (e.g., user.route.ts, product.route.ts)
...
### Database Setup
- Database schema and initial data in `server/database/query.sql`
...
6. **Navigation**: Update Navbar.tsx if creating new routes
...
- **Frontend**: File-based routing with nested routes
  ...
  - `products.edit.$id.tsx` → `/products/:id/edit`
  - `products.create.tsx` → `/products/create`
